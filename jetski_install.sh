...
REPO_URL="sso://ospo-internal/releasing/gemini-cloud-assist-skills/gemini-cloud-assist-skills"
DEST_DIR="$HOME/.gemini/jetski/skills"
SKILL_FOLDERS=("design-and-deploy")

echo "🚀 Starting Jetski Skill installation..."
...
echo "⚙️ Configuring Jetski MCP Server..."
if [ -x "$(dirname "$0")/jetski_configure_mcp.sh" ]; then
  "$(dirname "$0")/jetski_configure_mcp.sh"
else
  bash "$(dirname "$0")/jetski_configure_mcp.sh"
fi

echo "✅ Success! Your skills and MCP configurations are now installed."
echo "Refresh your Jetski agent to see new capabilities."